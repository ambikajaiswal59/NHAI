...
export default function Topography({ className = "" }) {
  ...
  return (
    <div className={`flex flex-col h-full w-full ${className}`}>
      ...
      <div className="relative z-[2000] w-full bg-white/90 backdrop-blur-sm rounded-lg shadow-sm border border-gray-200 p-1 mt-0 md:mt-3 lg:mt-0 mb-2 flex-shrink-0 overflow-visible">
        {" "}
        {/* Desktop: Grid View */}
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <span
                    className={`absolute -bottom-1 left-1/2 -translate-x-1/2 w-4 h-0.5 rounded-full ${item.dot}`}
                  />
                )}
              </button>
            );

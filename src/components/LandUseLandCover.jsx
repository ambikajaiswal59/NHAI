...
// Velocity color ranges for points
...
function getVelocityColor(velocity) {
  ...
  return "#8a0b68"; // Default color
}

// Horizontal Velocity Legend
function VelocityLegend() {
  return (
    <div className="absolute bottom-3 right-35 z-[1500] bg-white/95 backdrop-blur-sm rounded-md shadow-md border border-gray-200 px-4 py-2 max-w-[200px]">
      <div className="text-[10px] font-medium text-gray-500 text-center mb-1"></div>
      <div className="flex items-center gap-1">
        ...
        >
          {VELOCITY_RANGES.map((range, index) => {
            // Calculate percentage width for each range
            const totalRange = 100; // -50 to 50 = 100
...
export default function LandUseLandCover({
  ...
  const handleLayerChange = useCallback((layer) => {
    ...
    if (layer === "velocity") {
      ...
      setActiveLayers((prev) => {
        const newLayers = prev.filter(
          (id) => id !== "movement" && id !== "difference",
        );
        return newLayers;
      });
  ...
  const addMovementPointsToMap = useCallback(
    (map, points) => {
      ...
      points.forEach((feature) => {
        ...
        // Determine color based on velocity for velocity mode
        const fillColor =
          selectedLayer === "velocity" ? getVelocityColor(velocity) : "#8a0b68";
        const borderColor = selectedLayer === "velocity" ? "#333" : "#0d0101";
        ...
        circle.on("mouseout", function (e) {
          ...
          this.setStyle({
            ...
            color: selectedLayer === "velocity" ? "#333" : "#0d0101",
            weight: 1.5,
            fillOpacity: 0.85,
          });
          this.closeTooltip();
  ...
  // Update points when layer changes
  useEffect(() => {
      ...
      try {
        ...
        dividerReadyTimeoutRef.current = setTimeout(() => {
          ...
          if (dividerLineRef.current)
            dividerLineRef.current.style.left = `${pos}px`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Legends - Show both LULC and Velocity when Velocity is selected */}
        {!loading && !error && (
          <>
            <LULCLegend />

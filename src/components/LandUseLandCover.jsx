...
export default function LandUseLandCover({
  ...
  // Keeps the currently selected movement point highlighted
  // until another point is selected or the component is refreshed.
  ...
  const addMovementPointsToMap = useCallback(
    (map, points) => {
      ...
      // The movement circles are recreated when layer/date data changes,
      // so the previous selection must be cleared as well.
      ...
      points.forEach((feature) => {
        ...
        circle.on("mouseover", function () {
          // A selected point must remain selected even while the mouse
          // moves over it.
        ...
        circle.on("mouseout", function () {
          // Do not remove the persistent highlight from the selected point.
          if (selectedMovementMarkerRef.current === this) {
            this.setStyle(getSelectedCircleStyle(map.getZoom()));
          } else {
            this.setStyle(
        ...
        circle.on("click", async function () {
          ...
          // If the already-selected bubble is clicked again,
          // remove its highlight and clear selection
          if (selectedMovementMarkerRef.current === this) {
            this.setStyle(
              getRestingCircleStyle(
                selectedLayer,
            ...
            // Optional: close the chart/details
            setShowChart(false);
          ...
          // Remove highlight from previously selected bubble
  ...
  useEffect(() => {
    ...
    try {
      ...
      return () => {
        ...
        // Clear selected movement point when the map/component is destroyed.
        selectedMovementMarkerRef.current = null;
